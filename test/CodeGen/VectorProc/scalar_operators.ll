...
define i32 @addimm(i32 %a) { 	; CHECK: addimm:
	%1 = add i32 %a, 12 			; CHECK: s{{[0-9]+}} = s{{[0-9]+}} + 12
	ret i32 %1
}
...
define i32 @subimm(i32 %a) { 	; CHECK: subimm:
	%1 = sub i32 %a, 17 			; CHECK: s{{[0-9]+}} = s{{[0-9]+}} + -17
	ret i32 %1
}
...
define i32 @mulimm(i32 %a) { 	; CHECK: mulimm:
	%1 = mul i32 %a, 22 			; CHECK: s{{[0-9]+}} = s{{[0-9]+}} * 22
	ret i32 %1
}
...
define i32 @andimm(i32 %a) { 	; CHECK: andimm:
	%1 = and i32 %a, 27 			; CHECK: s{{[0-9]+}} = s{{[0-9]+}} & 27
	ret i32 %1
}
...
define i32 @orimm(i32 %a) { 	; CHECK: orimm:
	%1 = or i32 %a, 29 			; CHECK: s{{[0-9]+}} = s{{[0-9]+}} | 29
	ret i32 %1
}
...
define i32 @xorimm(i32 %a) { 	; CHECK: xorimm:
	%1 = xor i32 %a, 31			; CHECK: s{{[0-9]+}} = s{{[0-9]+}} ^ 31
	ret i32 %1
}
...
define i32 @shlimm(i32 %a) { 	; CHECK: shlimm:
	%1 = shl i32 %a, 7 			; CHECK: s{{[0-9]+}} = s{{[0-9]+}} << 7
	ret i32 %1
}
...
define i32 @ashrimm(i32 %a) { 	; CHECK: ashrimm:
	%1 = ashr i32 %a, 9 			; CHECK: s{{[0-9]+}} = s{{[0-9]+}} >> 9
	ret i32 %1
}
