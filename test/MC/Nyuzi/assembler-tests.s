...
or s21, s22, -94                 # CHECK: 0xb6,0x8a,0xfe,0x00
or v24, v25, 33                  # CHECK: 0x19,0x87,0x00,0x20
or_mask v27, s1, v0, 33          # CHECK: 0x60,0x87,0x10,0x60
...
and s22, s23, 26                 # CHECK: 0xd7,0x6a,0x00,0x01
and v25, v26, -66                # CHECK: 0x3a,0xfb,0xfe,0x21
and_mask v0, s2, v1, -21         # CHECK: 0x01,0x88,0xf5,0x61
...
xor s23, s24, 21                 # CHECK: 0xf8,0x56,0x00,0x03
xor v26, v27, 74                 # CHECK: 0x5b,0x2b,0x01,0x23
xor_mask v1, s3, v2, 66          # CHECK: 0x22,0x0c,0x21,0x63
...
add_i s24, s25, -103             # CHECK: 0x19,0x67,0xfe,0x05
add_i v27, v0, -115              # CHECK: 0x60,0x37,0xfe,0x25
add_i_mask v2, s4, v3, -2        # CHECK: 0x43,0x10,0xff,0x65
...
sub_i s25, s26, -1               # CHECK: 0x3a,0xff,0xff,0x06
sub_i v0, v1, -43                # CHECK: 0x01,0x54,0xff,0x26
sub_i_mask v3, s5, v4, -102      # CHECK: 0x64,0x14,0xcd,0x66
...
mull_i s26, s27, 97              # CHECK: 0x5b,0x87,0x01,0x07
mull_i v1, v2, -9                # CHECK: 0x22,0xdc,0xff,0x27
mull_i_mask v4, s6, v5, -49      # CHECK: 0x85,0x98,0xe7,0x67
...
mulh_u s27, s0, -80              # CHECK: 0x60,0xc3,0xfe,0x08
mulh_u v2, v3, 69                # CHECK: 0x43,0x14,0x01,0x28
mulh_u_mask v5, s7, v6, 54       # CHECK: 0xa6,0x1c,0x1b,0x68
...
ashr s0, s1, 36                  # CHECK: 0x01,0x90,0x00,0x09
ashr v3, v4, 65                  # CHECK: 0x64,0x04,0x01,0x29
ashr_mask v6, s8, v7, 114        # CHECK: 0xc7,0x20,0x39,0x69
...
shr s1, s2, -54                  # CHECK: 0x22,0x28,0xff,0x0a
shr v4, v5, -93                  # CHECK: 0x85,0x8c,0xfe,0x2a
shr_mask v7, s9, v8, 68          # CHECK: 0xe8,0x24,0x22,0x6a
...
shl s2, s3, -12                  # CHECK: 0x43,0xd0,0xff,0x0b
shl v5, v6, -44                  # CHECK: 0xa6,0x50,0xff,0x2b
shl_mask v8, s10, v9, 12         # CHECK: 0x09,0x29,0x06,0x6b
...
mulh_i s7, s8, 45                # CHECK: 0xe8,0xb4,0x00,0x1f
mulh_i v10, v11, -60             # CHECK: 0x4b,0x11,0xff,0x3f
mulh_i_mask v13, s15, v14, 2     # CHECK: 0xae,0x3d,0x01,0x7f
...
move s1, 72                      # CHECK: 0x21,0x20,0x01,0x0f
move v1, 72                      # CHECK: 0x21,0x20,0x01,0x2f
...
cmpeq_i s1, s2, 211              # CHECK: 0x22,0x4c,0x03,0x10
cmpeq_i s1, v2, 211              # CHECK: 0x22,0x4c,0x03,0x30
...
cmpne_i s4, s5, 112              # CHECK: 0x85,0xc0,0x01,0x11
cmpne_i s4, v5, 112              # CHECK: 0x85,0xc0,0x01,0x31
...
cmpgt_i s7, s8, 209              # CHECK: 0xe8,0x44,0x03,0x12
cmpgt_i s7, v8, 209              # CHECK: 0xe8,0x44,0x03,0x32
...
cmpge_i s10, s11, 8              # CHECK: 0x4b,0x21,0x00,0x13
cmpge_i s10, v11, 8              # CHECK: 0x4b,0x21,0x00,0x33
...
cmplt_i s13, s14, 199            # CHECK: 0xae,0x1d,0x03,0x14
cmplt_i s13, v14, 199            # CHECK: 0xae,0x1d,0x03,0x34
...
cmple_i s16, s17, 222            # CHECK: 0x11,0x7a,0x03,0x15
cmple_i s16, v17, 222            # CHECK: 0x11,0x7a,0x03,0x35
...
cmpgt_u s19, s20, 54             # CHECK: 0x74,0xda,0x00,0x16
cmpgt_u s19, v20, 54             # CHECK: 0x74,0xda,0x00,0x36
...
cmpge_u s22, s23, 1              # CHECK: 0xd7,0x06,0x00,0x17
cmpge_u s22, v23, 1              # CHECK: 0xd7,0x06,0x00,0x37
...
cmplt_u s25, s26, 164            # CHECK: 0x3a,0x93,0x02,0x18
cmplt_u s25, v26, 164            # CHECK: 0x3a,0x93,0x02,0x38
...
cmple_u s0, s1, 145              # CHECK: 0x01,0x44,0x02,0x19
cmple_u s0, v1, 145              # CHECK: 0x01,0x44,0x02,0x39
...
load_v v1, 260(s2)               # CHECK: 0x22,0x10,0x04,0xae
load_v_mask v1, s3, 260(s2)      # CHECK: 0x22,0x0c,0x82,0xb0
load_v v1, (s2)                  # CHECK: 0x22,0x00,0x00,0xae
load_v_mask v1, s3, (s2)         # CHECK: 0x22,0x0c,0x00,0xb0
store_v v1, 260(s2)              # CHECK: 0x22,0x10,0x04,0x8e
store_v_mask v1, s3, 260(s2)     # CHECK: 0x22,0x0c,0x82,0x90
store_v v1, (s2)                 # CHECK: 0x22,0x00,0x00,0x8e
store_v_mask v1, s3, (s2)        # CHECK: 0x22,0x0c,0x00,0x90
load_gath v4, 232(v5)            # CHECK: 0x85,0xa0,0x03,0xba
load_gath_mask v4, s6, 232(v5)   # CHECK: 0x85,0x18,0x74,0xbc
load_gath v4, (v5)               # CHECK: 0x85,0x00,0x00,0xba
load_gath_mask v4, s6, (v5)      # CHECK: 0x85,0x18,0x00,0xbc
store_scat v4, 232(v5)           # CHECK: 0x85,0xa0,0x03,0x9a
store_scat_mask v4, s6, 232(v5)  # CHECK: 0x85,0x18,0x74,0x9c
