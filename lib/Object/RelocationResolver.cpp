...
static bool supportsNyuzi(uint64_t Type) {
  switch (Type) {
  case ELF::R_NYUZI_ABS32:
    return true;
  default:
    return false;
  }
...
static uint64_t resolveNyuzi(RelocationRef R, uint64_t S, uint64_t A) {
  switch (R.getType()) {
  case ELF::R_NYUZI_ABS32:
    return S + getELFAddend(R);
  default:
...
getRelocationResolver(const ObjectFile &Obj) {
  if (Obj.isCOFF()) {
    ...
    case Triple::nyuzi:
      return {supportsNyuzi, resolveNyuzi};
    default:
      return {nullptr, nullptr};
    }
  } else if (Obj.isMachO()) {
