...
//
//===----------------------------------------------------------------------===//
#ifndef TARGET_VECTORPROC_VECTORPROCLDBACKEND_H
#define TARGET_VECTORPROC_VECTORPROCLDBACKEND_H
...
//===----------------------------------------------------------------------===//
/// VectorProcGNULDBackend - linker backend of VectorProc target of GNU ELF format
///
class VectorProcGNULDBackend : public GNULDBackend
{
public:
  static const int64_t VECTORPROC_MAX_FWD_BRANCH_OFFSET = (((1 << 25) - 1) << 2);
  static const int64_t VECTORPROC_MAX_BWD_BRANCH_OFFSET = (-((1 << 25) << 2));

public:
  VectorProcGNULDBackend(const LinkerConfig& pConfig, GNUInfo* pInfo);
...
public:
  ...
  VectorProcELFDynamic& dynamic();

  /// dynamic - the dynamic section of the target machine.
  /// Use co-variant return type to return its own dynamic section.
  const VectorProcELFDynamic& dynamic() const;
  ...
  /// getTargetSectionOrder - compute the layout order of VectorProc target sections
...
private:
  int64_t maxFwdBranchOffset() { return VECTORPROC_MAX_FWD_BRANCH_OFFSET; }
  int64_t maxBwdBranchOffset() { return VECTORPROC_MAX_BWD_BRANCH_OFFSET; }
...
private:
  ...
  // VectorProcELFAttributeData* m_pAttrData;

  VectorProcELFDynamic* m_pDynamic;

  //     variable name           :  ELF
  // LDSection* m_pAttributes;      // .ARM.attributes
  // LDSection* m_pPreemptMap;      // .VectorProc.preemptmap
  // LDSection* m_pDebugOverlay;    // .VectorProc.debug_overlay
  // LDSection* m_pOverlayTable;    // .VectorProc.overlay_table
};
} // namespace of mcld
