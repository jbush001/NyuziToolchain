...
void SPMDBuilder::pushMask(Value *MaskValue) {
  if (MaskStack.empty()) {
    ...
    Value *NewCombinedValue =
        Builder.CreateAnd(MaskValue, getCurrentMask(), "pred");
    MaskStackEntry Entry = {MaskValue, NewCombinedValue};
    MaskStack.push_back(Entry);
  }
}

void SPMDBuilder::popMask() { MaskStack.pop_back(); }
...
Value *SPMDBuilder::getCurrentMask() { return MaskStack.back().CombinedValue; }
